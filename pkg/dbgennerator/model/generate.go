...
func GenerateDbModel(tableName string) {
	...
	tableColArray, err := dbgennerator.ListTableColByTsANDTn("oss_storage", tableName)
	if err != nil {
		fmt.Println("数据查询错误")
	}
	...
	for _, col := range tableColArray {
		...
		tableColModelStruct := &tableColModel{
			...
			FiledName:       dbgennerator.UcFirst(dbgennerator.Case2Camel(col.ColumnName.String)),
			FiledType:       dbgennerator.FieldTypeMap[col.DataType.String],
	...
	for _, colModel := range tableColModelArray {
		...
		if temp := len(colModel.FiledType.SqlType); temp > filedTypeMaxLength {
	...
	for _, colModel := range tableColModelArray {
		...
		tempFiledTypeByte := []byte(colModel.FiledType.GoType)
